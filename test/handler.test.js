...
describe('HoldRequestConsumer Lambda: Handle Kinesis Stream Input', () => {
  ...
  describe('Kinesis Handler: exports.kinesisHandler()', () => {
    ...
    it('should return null if no apiUri param is defined', () => {
      const result = kinesisHandlerFunc(event.Records, { schema: 'Bib' }, null);
      const result2 = kinesisHandlerFunc(event.Records, { schema: 'Bib', apiUri: undefined }, null);
      expect(result).to.equal(null);
      expect(result2).to.equal(null);
    });

    it('should return null if the apiUri param is an empty string', () => {
      const result = kinesisHandlerFunc(event.Records, { schema: 'Bib', apiUri: '' }, null);
      expect(result).to.equal(null);
    });

    it('should return a promise if the proper event.Records, schema and apiUri are defined', () => {
      const result = kinesisHandlerFunc(event.Records, { schema: 'Bib', apiUri: 'https://api.nypltech.org/api/v0.1/' }, null);
